...
class BouquetsWriter():
	...
	def buildBouquets(self, path, provider_config, services, sections, section_identifier, preferred_order, bouquets_to_hide, section_prefix):
		section_sep = " - "
		if len(section_prefix) > 0: section_prefix += section_sep
		current_number = 0
		...
		if provider_config.isMakeNormalMain():
			...
			current_bouquet_list.append("#NAME %s\n" % section_prefix.removesuffix(section_sep) if len(section_prefix) > 0 and provider_config.isMakeNormalMainOnly() else section_prefix + _('All channels'))
